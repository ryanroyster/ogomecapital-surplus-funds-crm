...
module.exports=function({cfg,fetchJson,serviceHeaders,requireUser,readBody,json,dbGet}){
 ...
 async function leads(user,items){
  ...
  const changed=[];
  for(const item of items){const previous=map.get(String(item.id));if(previous&&Number(item._automation_revision||0)!==Number(previous._automation_revision||0))fail('Lead changed through automation. Download cloud changes before saving; your device copy retains unsynced edits.',409);const next=domain.compliance({...previous,...item});try{domain.validate(next,previous);}catch(e){fail(e.message,422);}if(JSON.stringify(previous)!==JSON.stringify(next))changed.push({previous,next});}
